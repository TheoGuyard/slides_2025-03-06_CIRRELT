...
\begin{frame}[fragile]{BnB -- Hands-on with \texttt{el0ps}}
    \begin{tikzpicture}[remember picture,overlay]
        \begin{scope}[xshift=0.5\linewidth]
            \node[text width=0.9\linewidth,align=center] at (1.75,-0.5) {
                \begin{lstlisting}
                ...
                # Solve the problem with el0ps' solver
                solver = BnbSolver()
                solver.solve(f, h, lmbd=0.01)
                \end{lstlisting}
            };
            %
            %
            %
            \onslide<1-> {
                \node[text width=0.4\linewidth,align=center] (reg) at (-3.5,1) {\textbf{Sparse regression} \\ Find $\pv$ sparse such that $\obs \simeq \dic\pv$};
            }
            %
            %
            %
            ...
            %
            \onslide<3-> {
                \node[fill=mDarkTeal] at (2.5,2.5) {\textcolor{TolLightWhite}{\texttt{\$ pip install el0ps}}};
            }
            %
            %
            %
            \onslide<1-3> {
                \fill[TolLightWhite] (-1,2) rectangle (6,-0.5);
            }
            %
            %
            %
            \onslide<1-4> {
                \fill[TolLightWhite] (-1,-0.6) rectangle (6,-1.75);
            }
            %
            %
            %
            \onslide<1-5> {
                \fill[TolLightWhite] (-1,-1.8) rectangle (6,-3.5);
            }
            %
            %
            %
